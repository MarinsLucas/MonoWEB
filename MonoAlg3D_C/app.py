...
library_file_options = ["shared_libs/libToRORd_dynCl_mixed_endo_mid_epi.so", "shared_libs/libmitchell_shaeffer_2003.so", "shared_libs/libstewart_aslanidi_noble_2009.so", "shared_libs/libten_tusscher_2006.so", "shared_libs/libohara_rudy_endo_2011.so",
                        ...
                        "shared_libs/libToRORd_fkatp_endo.so", "shared_libs/libten_tusscher_3_endo.so", "shared_libs/libten_tusscher_2004_endo.so", "shared_libs/libToRORd_Land_mixed_endo_mid_epi.so", "shared_libs/libten_tusscher_tt3_mixed_endo_mid_epi.so" ]
...
def ex_options_available():
    dir_outputs = "./outputs"
    dir_configs = "./example_configs"
    example_nums = [nome for nome in os.listdir(dir_outputs) if os.path.isdir(os.path.join(dir_outputs, nome))]
    examples_options = []

    for pasta in example_nums:
        for arquivo in os.listdir(dir_configs):
            if arquivo.startswith(pasta):
                examples_options.append(arquivo)
    examples_options.sort()
    return examples_options

examples_options = ex_options_available()
...
def load_data(nf):
    ...
    simple.ResetCamera()
    view.CenterOfRotation = view.CameraFocalPoint
    tempos, vm = ler_dados_AP('./outputs/'+nf+"/Vm_matrix.txt")
    fig.data = []
    fig.add_trace(go.Scatter(x = tempos, y=vm, mode='lines', name="Vm"))
    fig.update_layout(title="Transmembrane Potential", xaxis_title="Time(ms)", yaxis_title="Vm(mV)")
...
def readini(nome_arquivo):
    ...
    #Extra data
    if state.extra_data:
        state.extra_data_selected = config["extra_data"]['main_function']
        ...
        if state.extra_data_selected == "set_extra_data_for_fibrosis_sphere":
            ...
            state.atpi = config["extra_data"].get("atpi",6.4)
            state.Ko = config["extra_data"].get("Ko",5.4)
            state.Ki = config["extra_data"].get("Ki",138.3)
            state.GNa_multiplicator = config["extra_data"].get("GNa_multiplicator", 1)
            state.GCaL_multiplicator = config["extra_data"].get("GCaL_multiplicator",1)
            state.INaCa_multiplicator = config["extra_data"].get("INaCa_multiplicator",1)
            state.Vm_modifier = config["extra_data"].get("Vm_modifier",0)
            state.Ikatp_multiplicator = config["extra_data"].get("Ikatp_multiplicator",1)
    ...
    with SinglePageWithDrawerLayout(server) as layout:
        ...
        with layout.content:
            with vuetify.VContainer(fluid=True,classes="fill-height"):
                ...
                with vuetify.VCol(style="max-width: 50%",classes="ma-0 fill-height",  align ="end", cols=6, sm=6):
                        global plot_view
